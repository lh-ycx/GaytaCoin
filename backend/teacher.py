...
class Teacher_Manager(object):
    ...
    def addTeacher(self, teacherId, password, teacherName):
        ...
        self.teacherId2id = Teacher.insert_one(
            {"teacherId": teacherId, "password": password, "teacherName": teacherName}).inserted_id
    ...
    def resetPassword(self, teacherId, new_password, old_password):
        ...
        if old_password == res['password']:
            self.db.Teacher.update({'teacherId': teacherId}, {
    ...
    def addCourse(self, teacherId, course_name):
        ...
        self.db.Courses.insert_one(
            {"courseName": course_name, "courseId": candidateId, "teacherId": teacherId})

        # 创建房间
        room = self.db.Room
        room.insert_one({"room_id": candidateId, "name": course_name, "owner": teacherId,
                         "users": [], "messages": []})
        return json.dumps({"response_code": 1})
    ...
    def deleteCourseByName(self, teacherId, course_name):
        ...
        # 首先更新Teacher表
        self.db.Teacher.update({'teacherId': teacherId}, {
                               "$set": {'courses': res.remove(course_name)}})
    ...
    def deleteCourseById(self, teacherId, courseId):
        ...
        # 最后删除聊天室
        res = self.db.Room.find_one({"room_id": courseId})
        if res is None:
    ...
    def getRegisterListbyCourseId(self, courseId):
        res = self.db.Register.find(
            {"courseId": courseId}).sort([("registerId", 1)])
